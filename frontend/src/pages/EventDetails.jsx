...
//   const handleRegister = () => dispatch(registerVolunteer(id, navigate));
//   const handleUnregister = () => dispatch(unregisterVolunteer(id, navigate));

//   const handleVolunteerStatusChange = (volunteerId, status) => {
//     dispatch(updateVolunteerStatus(volunteerId, id, status));
//     setEvent(prevEvent => {
//       const updatedVolunteers = prevEvent.volunteers.map(volunteer =>
//         volunteer.volunteerId._id === volunteerId ? { ...volunteer, status } : volunteer
...
//   if (!event) return <div className="text-center py-10 text-white">Loading...</div>;

//   return (
//     <div className="w-full px-4 sm:px-6 lg:px-10 max-w-7xl mx-auto py-10 bg-richblack-700 text-white rounded-md">
//       <h1 className="text-2xl sm:text-3xl lg:text-4xl text-center uppercase mb-6">
...
//         src={event.image}
//         alt="event"
//         className="w-full max-h-[400px] object-cover rounded-lg mb-6 shadow-lg"
//       />

//       {/* Event Info Section */}
//       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm sm:text-base text-richblack-25 mb-8">
//         <div>
//           <p><strong>Type of Food:</strong> {event.typeOfFood}</p>
//           <p className="mt-1"><strong>Location:</strong> {event.location}</p>
//         </div>
//         <div>
//           <p><strong>Quantity:</strong> {event.quantity}</p>
//           <p className="mt-1"><strong>Contact:</strong> {event.contactDetails}</p>
//         </div>
//         <div>
//           <p><strong>Event Date:</strong> {formatDate(event.eventDate)}</p>
//           <p className="mt-1"><strong>Status:</strong> {event.status}</p>
//         </div>
//       </div>
...
//           {event.volunteers.some(v => v.volunteerId._id === token.userId && v.status === 'accepted') ? (
//             <p className="text-caribbeangreen-300 text-base sm:text-lg font-bold uppercase">
//               You are accepted for this event.
//             </p>
//           ) : (
//             <div className="flex flex-col sm:flex-row justify-center gap-4">
//               <button
//                 className="bg-caribbeangreen-500 text-white py-2 px-4 sm:px-6 rounded font-bold w-full sm:w-auto hover:scale-105 transition"
//                 onClick={handleRegister}
//               >
//                 Register
//               </button>
//               <button
//                 className="bg-pink-600 text-white py-2 px-4 sm:px-6 rounded font-bold w-full sm:w-auto hover:scale-105 transition"
...
//           <h2 className="text-xl sm:text-2xl font-bold mb-4 text-center">Volunteers</h2>
//           <div className="space-y-4">
//             {event.volunteers.map(volunteer => (
//               <div
//                 key={volunteer.volunteerId._id}
//                 className="bg-richblack-800 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
//               >
//                 <div>
//                   <p className="font-semibold text-base">{volunteer.volunteerId.name}</p>
//                   <p className="text-sm text-richblack-100">{volunteer.volunteerId.email}</p>
//                 </div>
//                 {volunteer.status === 'accepted' || volunteer.status === 'rejected' ? (
//                   <p className={`font-semibold uppercase text-sm sm:text-base ${volunteer.status === 'accepted' ? 'text-caribbeangreen-100' : 'text-pink-200'}`}>
//                     {volunteer.status}
//                   </p>
//                 ) : (
//                   <div className="flex gap-2 flex-wrap">
//                     <button
//                       className="bg-caribbeangreen-400 text-white py-1.5 px-4 rounded hover:scale-105 text-sm"
//                       onClick={() => handleVolunteerStatusChange(volunteer.volunteerId._id, 'accepted')}
//                     >
//                       Approve
//                     </button>
//                     <button
//                       className="bg-pink-500 text-white py-1.5 px-4 rounded hover:scale-105 text-sm"
//                       onClick={() => handleVolunteerStatusChange(volunteer.volunteerId._id, 'rejected')}
//                     >
//                       Disapprove
//                     </button>
...
//               <button
//                 className="bg-pink-300 text-black py-2 px-6 rounded font-semibold text-sm sm:text-base hover:scale-110 transition"
